...
if (!process.env.ADMIN_USER) {
  throw new Error('ADMIN_USER devem estar configurados no ambiente.');
}

if (!process.env.ADMIN_PASSWORD) {
  throw new Error(' ADMIN_PASSWORD devem estar configurados no ambiente.');
}

if (!process.env.SESSION_SECRET) {
  throw new Error('SESSION_SECRET devem estar configurados no ambiente.');
}
...
app.post('/api/login', async (req,res) => {
  ...
  try {
    if (login === process.env.ADMIN_USER.toLowerCase() && senha === process.env.ADMIN_PASSWORD) {
...
app.get('/',(req,res)=>res.sendFile(path.join(process.cwd(),'..','app','login.html')));
app.get('/login',(req,res)=>res.sendFile(path.join(process.cwd(),'..','app','login.html')));
app.get('/agendamento',(req,res)=>res.sendFile(path.join(process.cwd(),'..','app','agendamento.html')));
app.get('/admin',(req,res,next)=>req.session?.papel==='admin'||req.session?.papel==='visualizador'?res.sendFile(path.join(process.cwd(),'..','app','admin.html')):res.redirect('/login.html'));
app.get('/admin.html',(req,res,next)=>req.session?.papel==='admin'||req.session?.papel==='visualizador'?res.sendFile(path.join(process.cwd(),'..','app','admin.html')):res.redirect('/login.html'));
app.get('/cadastro',(req,res)=>res.sendFile(path.join(process.cwd(),'..','app','cadastro.html')));
app.get('/cidadao',(req,res)=>req.session?.papel==='cidadao'?res.sendFile(path.join(process.cwd(),'..','app','cidadao.html')):res.redirect('/login.html'));
app.get('/cidadao.html',(req,res)=>req.session?.papel==='cidadao'?res.sendFile(path.join(process.cwd(),'..','app','cidadao.html')):res.redirect('/login.html'));
app.use(express.static(path.join(process.cwd(),'..','app')));
app.use((req,res,next)=>{if(req.method==='GET'&&!req.path.startsWith('/api/'))return res.sendFile(path.join(process.cwd(),'..','app','login.html'));next();});
